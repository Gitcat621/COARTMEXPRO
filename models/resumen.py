...
class Resumen:
    ...
    def listar_resumenAnalisis(meses_str, year):
        ...
        (COALESCE((SELECT SUM(f.totalFactura) 
            ...
            WHERE f.fechaPagado IS NOT NULL 
            AND YEAR(f.fechaPagado) = {year} 
            AND MONTH(f.fechaPagado) IN ({meses})), 0) 
        - 
        (COALESCE((SELECT SUM(g.montoGasto) 
            FROM gastos g 
            WHERE YEAR(g.fechaGasto) = {year} 
        ...
        COALESCE((SELECT SUM(cm.montoMercancia) 
            FROM compras_mercancia cm 
            WHERE YEAR(cm.fechaMercancia) = {year} 
            AND MONTH(cm.fechaMercancia) IN ({meses})), 0))) AS efectivoRestante,

        -- Total de ventas
        COALESCE((SELECT SUM(v.montoVenta) 
            FROM ventas v 
            WHERE YEAR(v.fechaVenta) = {year}
                AND MONTH(v.fechaVenta) IN ({meses})), 0) AS totalVenta,

        -- Comparación de ventas año actual vs año pasado
        COALESCE((SELECT SUM(CASE WHEN YEAR(v.fechaVenta) = {year} 
            AND MONTH(v.fechaVenta) IN ({meses}) THEN v.montoVenta ELSE 0 END) 
        ...
        COALESCE((SELECT SUM(a.precioAlmacen * e.cantidadExistencia) 
            ...
            AND MONTH(e.fechaExistencia) IN ({meses})), 0) AS valorInventario,
        ...
        COALESCE((SELECT SUM(f.totalFactura) 
            ...
            AND MONTH(f.fechaFactura) IN ({meses})), 0) AS totalCxC,
        ...
        COALESCE((SELECT SUM(cm.montoMercancia) 
            ...
            AND MONTH(cm.fechaMercancia) IN ({meses})), 0) AS totalCxP,
        ...
        -- Compras de mercancia
         COALESCE((SELECT SUM(cm.montoMercancia) 
            ...
            AND MONTH(cm.fechaMercancia) IN ({meses})), 0) AS totalComprasMercancia,
        ...
        COALESCE((SELECT SUM(g.montoGasto) 
            FROM gastos g 
            WHERE YEAR(g.fechaGasto) = {year} 
            AND MONTH(g.fechaGasto) IN ({meses})), 0) AS totalGastos,
        ...
        COALESCE((SELECT SUM(e.cantidadExistencia) 
            ...
            JOIN existencias e ON e.fkCodigoArticulo = a.codigoArticulo 
            WHERE YEAR(e.fechaExistencia) = {year}  
            AND MONTH(e.fechaExistencia) IN ({meses})), 0) AS existencias,
        ...
        COALESCE((SELECT COUNT(DISTINCT sc.nombreSocio) 
            ...
            AND MONTH(oc.fechaOrdenCompra) IN ({meses})), 0) AS sociosNegociados,
        ...
        COALESCE((SELECT COUNT(av.fkCodigoArticulo) 
            ...
            JOIN ordenes_compra oc ON oc.pkOrdenCompra = av.fkOrdenCompra 
            WHERE YEAR(oc.fechaOrdenCompra) = {year} 
            AND MONTH(oc.fechaOrdenCompra) IN ({meses})), 0) AS articulosVendidos,

        MAX(GREATEST(
            COALESCE((SELECT MAX(f.fechaPagado) FROM facturas f WHERE f.fechaPagado IS NOT NULL AND MONTH(f.fechaPagado) IN ({meses}) AND YEAR(f.fechaPagado) = {year}), '0000-00-00'),
            COALESCE((SELECT MAX(g.fechaGasto) FROM gastos g WHERE MONTH(g.fechaGasto) IN ({meses}) AND YEAR(g.fechaGasto) = {year}), '0000-00-00'),
            COALESCE((SELECT MAX(cm.fechaMercancia) FROM compras_mercancia cm WHERE MONTH(cm.fechaMercancia) IN ({meses}) AND YEAR(cm.fechaMercancia) = {year}), '0000-00-00'),
            COALESCE((SELECT MAX(v.fechaVenta) FROM ventas v WHERE MONTH(v.fechaVenta) IN ({meses}) AND YEAR(v.fechaVenta) = {year}), '0000-00-00'),
            COALESCE((SELECT MAX(oc.fechaOrdenCompra) FROM ordenes_compra oc WHERE MONTH(oc.fechaOrdenCompra) IN ({meses}) AND YEAR(oc.fechaOrdenCompra) = {year}), '0000-00-00'),
            COALESCE((SELECT MAX(e.fechaExistencia) FROM existencias e WHERE MONTH(e.fechaExistencia) IN ({meses}) AND YEAR(e.fechaExistencia) = {year}), '0000-00-00')
