...
class Factura:
    ...
    def listar_facturas():
        ...
        db = Database()
        consulta = '''
        SELECT f.numeroAnio, f.totalFactura, f.fechaVencimiento, f.diasVencidos, f.fechaPagado, sc.nombreSocio FROM facturas f 
    ...
    def crear_factura(self, db):
        ...
        INSERT INTO facturas (
            fechaFactura, numeroAnio, subTotalFactura, totalFactura,
        ...
        try:
            ...
            guardar_en_log(f"❌ Error al insertar la factura: {self.numeroAnio}: {e}")
            return None

    def editar_factura(self, db):
        """Edita un registro en la base de datos."""

        query = "UPDATE facturas SET fechaPagado = %s WHERE numeroAnio = %s AND YEAR(fechaFactura) = YEAR(%s)"

        valores = (self.fechaPagado, self.numeroAnio, self.fechaPagado)
        ...
        try:
            ...
            guardar_en_log(f"❌ Error al insertar compra {self.numeroAnio}: {e}")
            return None
