...
async function listarArticulos(pkCaja) {
    try {
        ...
        const cantidadResponse = await fetch(`http://127.0.0.1:5000/coartmex/cajas_contenido?pkCaja=${pkCaja}`);
...
async function listarCajas() {
    ...
    try {
        ...
        const response = await fetch(`http://127.0.0.1:5000/coartmex/cajas?pkEnvio=${pkEnvio}`, {
            method: 'GET',
...
async function agregarCaja() {
    try {
        ...
        const response = await fetch('http://127.0.0.1:5000/coartmex/cajas', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
