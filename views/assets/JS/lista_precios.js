...
async function obtenerTienda(){
  try {
    ...
    const response = await fetch(`http://127.0.0.1:5000/coartmex/socioComercial?pkSocioComercial=${pkSocioComercial}`, {
        method: 'GET',
...
async function listarArticulos() {
    try {
        ...
        const preciosResponse = await fetch(`http://127.0.0.1:5000/coartmex/lista_precios?fkSocioComercial=${fkSocioComercial}`);
...
$("#switch-1").on("change", function () {
    ...
    toastr.info(`El modo de guardado es: ${mensaje}`, 'Atención', { "closeButton": true });
});
...
async function agregarListaPrecio() {
  ...
  const response = await fetch(`http://127.0.0.1:5000/coartmex/listas_precios`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
