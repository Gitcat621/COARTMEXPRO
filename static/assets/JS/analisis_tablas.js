...
async function cargarTablas(endpoint, grupo, anio, mes) {
    ...
    if(endpoint == "Gastos" || endpoint == "Ventas"){
        endpoint = endpoint.charAt(0).toLowerCase() + endpoint.slice(1);
    }
    if(endpoint == "Ingresos"){
       endpoint = '/analisis/ingresos'
    }
    ...
    url = `/api/${endpoint}?grupo=${grupo}&year=${anio}&month=${mes}`;
    console.log(url);

    try {
        const response = await fetch(`/api/${endpoint}?grupo=${grupo}&year=${anio}&month=${mes}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
