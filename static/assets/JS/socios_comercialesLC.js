...
$(document).on('click', '.articulos-btn', function () {
    ...
    window.location.href = `./lista_precios?lista=${encodeURIComponent(listaID)}`;
});
...
async function listarsocios_comerciales() {
    try {
        const response = await fetch('/api/socios_comerciales', {
            method: 'GET',
