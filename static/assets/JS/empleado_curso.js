...
async function listarCursos() {
    try {
        ...
        toastr.error('La petición de cursos de empleado no se pudo concretar', 'Error', {"closeButton": true,});
    }
}
...
async function agregarCursoEmpleado() {
    ...
    try {
        ...
        const response = await fetch('/api/empleados/asistencia_cursos', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function eliminarCurso(pkAsistenciaCurso) {
    try {
        ...
        const response = await fetch('/api/empleados/cursos_empleado', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
