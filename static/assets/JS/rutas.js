...
async function chatGtp(fkZonaRuta,pkRuta) {
    ...
    fetch(`/api/rutas/destinos?pkRuta=${pkRuta}`)
    ]);
...
async function listarTiendas(fkZonaRuta) {
    try {
        ...
        const response = await fetch(`/api/tiendas?fkZonaRuta=${fkZonaRuta}`, {
            method: 'GET',
...
async function listarDestinosEmpleado(pkRuta) {
    try {
        ...
        const response = await fetch(`/api/destinos?pkRuta=${pkRuta}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        });
