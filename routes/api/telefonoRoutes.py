...
api_telefonos = Blueprint('api_telefonos', __name__)

@api_telefonos.route('/', methods=['GET'])
def listar_telefonos():
    """Endpoint para obtener todos los registros"""
    telefonos = Telefono.listar_telefonos()
    return jsonify(telefonos), 200

@api_telefonos.route('/', methods=['POST'])
def crear_telefono():
...
@api_telefonos.route('/', methods=['PUT'])
def editar_telefono():
...
@api_telefonos.route('/', methods=['DELETE'])
