...
api_cursos = Blueprint('api_cursos', __name__)

@api_cursos.route('/', methods=['GET'])
def listar_cursos():
    """Endpoint para obtener todos los registros"""
    cursos = Curso.listar_cursos()
    return jsonify(cursos), 200

@api_cursos.route('/', methods=['POST'])
def crear_curso():
...
@api_cursos.route('/', methods=['DELETE'])
def eliminar_curso():
