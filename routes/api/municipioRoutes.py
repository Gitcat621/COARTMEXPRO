...
api_municipios = Blueprint('api_municipios', __name__)

@api_municipios.route('/', methods=['GET'])
def listar_municipios():
    """Endpoint para obtener todos los registros"""
    municipios = Municipio.listar_municipios()
    return jsonify(municipios), 200

@api_municipios.route('/', methods=['POST'])
def crear_municipio():
...
@api_municipios.route('/', methods=['PUT'])
...
@api_municipios.route('/', methods=['DELETE'])
