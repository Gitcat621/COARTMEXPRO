...
api_clinicas = Blueprint('api_clinicas', __name__)

# Rutas endpoints
@api_clinicas.route('/', methods=['GET'])
def listar_clinicas():
    """Endpoint para obtener todas las clínicas"""
    clinicas = Clinica.listar_clinicas()
    return jsonify(clinicas), 200

@api_clinicas.route('/', methods=['POST'])
def crear_clinica():
...
@api_clinicas.route('/', methods=['PUT'])
def editar_clinica():
...
@api_clinicas.route('/', methods=['DELETE'])
