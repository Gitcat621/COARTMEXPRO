...
api_prestamos = Blueprint('api_prestamos', __name__)

# Rutas endpoints
@api_prestamos.route('/', methods=['GET'])
def listar_prestamos():
    """Endpoint para obtener todos los préstamos."""
    prestamos = Prestamo.listar_prestamos()
    return jsonify(prestamos), 200

@api_prestamos.route('/', methods=['POST'])
def crear_prestamo():
...
@api_prestamos.route('/', methods=['PUT'])
def editar_prestamo():
...
@api_prestamos.route('/', methods=['DELETE'])
