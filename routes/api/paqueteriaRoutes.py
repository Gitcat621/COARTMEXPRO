...
api_paqueterias = Blueprint('api_paqueterias', __name__)

@api_paqueterias.route('/', methods=['GET'])
def listar_paqueterias():
    """Endpoint para obtener todos los registros"""
    paqueterias = Paqueteria.listar_paqueterias()
    return jsonify(paqueterias), 200

@api_paqueterias.route('/', methods=['POST'])
...
@api_paqueterias.route('/', methods=['PUT'])
...
@api_paqueterias.route('/', methods=['DELETE'])
