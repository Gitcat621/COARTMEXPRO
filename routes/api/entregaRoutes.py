...
api_entregas = Blueprint('api_entregas', __name__)

@api_entregas.route('/', methods=['GET'])
def listar_entregas():
    """Endpoint para obtener todos los registros"""
    entregas = Entrega.listar_entregas()
    return jsonify(entregas), 200

@api_entregas.route('/', methods=['POST'])
def crear_entrega():
...
@api_entregas.route('/', methods=['PUT'])
def editar_entrega():
...
@api_entregas.route('/', methods=['DELETE'])
