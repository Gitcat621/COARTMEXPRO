...
api_bancos = Blueprint('api_bancos', __name__)

@api_bancos.route('/', methods=['GET'])
def listar_bancos():
    """Endpoint para obtener todos los registros"""
    bancos = Banco.listar_bancos()
    return jsonify(bancos), 200

@api_bancos.route('/', methods=['POST'])
def crear_banco():
...
@api_bancos.route('/', methods=['DELETE'])
def eliminar_banco():
