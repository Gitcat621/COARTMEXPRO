...
api_facturas = Blueprint('api_facturas', __name__)

@api_facturas.route('/', methods=['GET'])
def listar_facturas():
    """Endpoint para obtener todos los registros"""
    facturas = Factura.listar_facturas()
    return jsonify(facturas), 200

@api_facturas.route('/', methods=['POST'])
def crear_factura():
...
@api_facturas.route('/', methods=['PUT'])
def editar_factura():
...
@api_facturas.route('/', methods=['DELETE'])
