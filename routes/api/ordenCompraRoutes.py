...
api_ordenesCompra = Blueprint('api_ordenesCompra', __name__)

@api_ordenesCompra.route('/', methods=['GET'])
def listar_ordenesCompra():
    """Endpoint para obtener todos los registros"""
    ordenesCompra = OrdenCompra.listar_ordenesCompra()
    return jsonify(ordenesCompra), 200

@api_ordenesCompra.route('/', methods=['POST'])
...
@api_ordenesCompra.route('/', methods=['PUT'])
...
@api_ordenesCompra.route('/', methods=['DELETE'])
