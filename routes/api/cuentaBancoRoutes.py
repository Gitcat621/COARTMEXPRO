...
api_cuentasBanco = Blueprint('api_cuentasBanco', __name__)

@api_cuentasBanco.route('/', methods=['GET'])
def listar_cuentas():
    """Endpoint para obtener todos los registros"""
    cuentas = CuentaBanco.listar_cuentas()
    return jsonify(cuentas), 200

@api_cuentasBanco.route('/', methods=['POST'])
def crear_cuenta():
...
@api_cuentasBanco.route('/', methods=['PUT'])
def editar_cuenta():
...
@api_cuentasBanco.route('/', methods=['DELETE'])
def eliminar_cuenta():
